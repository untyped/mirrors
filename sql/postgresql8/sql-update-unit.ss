...
(define (drop-sql table cascade?)
  ...
  (format "DROP TABLE IF EXISTS ~a~a; DROP SEQUENCE IF EXISTS ~a;"
          (escape-name table-name)
          (if cascade? " CASCADE" "")
          (escape-name sequence-name)))
...
(define (update-sql struct)
  (define guid (struct-guid struct))
  ...
  (if (struct-saved? struct)
      (string-append "UPDATE " (escape-name (entity-table-name entity)) " SET "
                     (string-join (map (lambda (attr value)
                                  ...
                                  ", ")
                     " WHERE " (escape-name 'id) " = " (escape-value (entity-id-type entity) guid) ";")
...
(define (delete-sql guid)
  ...
  (define name (entity-table-name entity))
...
(define (check-insert-structs entity structs [skip-id-check? #f])
  (for-each (lambda (struct)
              ...
              ; void | exn:fail:snooze
              (when (and (struct-saved? struct) (not skip-id-check?))
                (raise-exn exn:fail:snooze
                  (format "Struct must be unsaved to perform an INSERT: ~s" struct))))
            structs))
